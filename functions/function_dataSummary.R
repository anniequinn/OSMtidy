...
source("functions/functions_internal.R", local = TRUE)
  ...
  # dataInput ---------------------------------------------------------------
  # -------------------------------------------------------------------------
  if(class(data) %in% c("OSMtidy_dataInput") %>% sum) {
    ...
    dt <- 
      ...
      lapply(data, function(x) { 
        ...
        n = y %>% nrow()
          ...
          map_dfr(~ .x %>% as_tibble(), .id = "name") %>% rename(feature = name) 
      
    }) %>% 
      map_dfr(~ .x %>% as_tibble(), .id = "name") %>% rename(type = name)
    
    output <- 
      list(class = class(data),
           byGeometry = dt %>% group_by(type) %>% summarise(total = sum(value, na.rm = TRUE)) %>% ungroup %>% as.data.frame,
           byFeature = dt %>% group_by(feature) %>% summarise(total = sum(value, na.rm = TRUE)) %>% ungroup %>% as.data.frame)
  ...
  if(class(data) %in% c("OSMtidy_dataCut") %>% sum) {
    ...
    output <- 
      list(class = class(data),
           byGeometry = dt %>% group_by(type) %>% summarise(total = sum(total, na.rm = TRUE)) %>% ungroup %>% as.data.frame,
           byFeature = dt %>% group_by(feature) %>% summarise(total = sum(total, na.rm = TRUE)) %>% ungroup %>% as.data.frame)
